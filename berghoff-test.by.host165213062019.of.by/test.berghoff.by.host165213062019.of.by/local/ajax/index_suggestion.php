...
$house = preg_replace("/[^0-9]/",'',$_REQUEST["house"]);
if($street && $CityID){
    ...
    $address = "г.".$arLocs["CITY_NAME"]." ул.".$street." д.".$house;
    $rsINDEX = $YSuggest->getIndex( $address );

    if($rsINDEX["data"]["value"]){
        $arResult["TYPE"] = "OK";
        $arResult["INDEX"] = $rsINDEX["data"]["value"];
    }else{
