			...
			<div class="catalog-detail-pictures--">
				...
				<div class="catalog-detail-picture" id="<?=$arItemIDs['PICT']?>">
					...
					//DETAIL_PICTURE//
                        ...
                        $array_pict =array();
					   $array_pict = $arResult["MORE_PHOTO"];
					    array_push($array_pict,$arResult['DETAIL_PICTURE']);
//                        $array_pict = array_reverse($array_pict);
					    ?>
...
//                            if(count($arResult["MORE_PHOTO"]) > 0) {
                                foreach($array_pict as $key_photo=>$PHOTO1) {?>
                                	<?if(!empty($PHOTO1)):?>
                                    <div class="catalog-detail-images">
                                        <img src="<?=$PHOTO1['SRC']?>" data-src="<?=$PHOTO1['SRC']?>" alt="<?=$arResult['NAME']?>" title="<?=$arResult['NAME']?>" />
                                    </div>
                                    <?endif;?>
                                <?}
//                            }
...
<!--					--><?//}?>
					<!-- <div class="more_photo" style="display:none;"> -->
					<div class="more_photo">
						<?if(!empty($arResult["PROPERTIES"]["VIDEO"]["VALUE"])) {?>
							<li class="catalog-detail-video" style="<?=($arParams['DISPLAY_MORE_PHOTO_WIDTH'] ? 'width:'.$arParams['DISPLAY_MORE_PHOTO_WIDTH'].'px;' : '').($arParams['DISPLAY_MORE_PHOTO_HEIGHT'] ? 'height:'.$arParams['DISPLAY_MORE_PHOTO_HEIGHT'].'px;' : '');?>">
								<div rel="lightbox" class="catalog-detail-images fancybox" href="#video">
						...
						<?}
//						if(count($arResult["MORE_PHOTO"]) > 0) {
							foreach($array_pict as $key_photo=>$PHOTO2) {?>
                                	<?if(!empty($PHOTO2)):?>
									<div class="catalog-detail-images">
										<img src="<?=$PHOTO2['SRC']?>" data-src="<?=$PHOTO2['SRC']?>" width="100px" alt="<?=$arResult['NAME']?>" title="<?=$arResult['NAME']?>" />
...
<!--									</span>-->
								<?} else {
									...
									<?}?>
									<span class="catalog-detail-item-price">
										<span class="catalog-detail-item-price-current <?if($arResult['MIN_PRICE']['BASE_PRICE']!=$arResult['MIN_PRICE']['PRICE']) {echo 'skidka';}?>">
											<?if($arResult["COLLECTION"]["THIS"]) {?>
												<span class="from"><?=GetMessage("CATALOG_ELEMENT_FROM");?></span>
											<?}?>
...
//										    pr($arResult['PROPERTIES']);
                                            if($arResult['PROPERTIES']['NOT_QUANTITY']['VALUE'] =="Нет в наличии" ){?>
                                                <p class="not-available"><?=GetMessage("CT_BCS_ELEMENT_NOT_AVAILABLE")?></p>
                                            <?}else{
                                                //DETAIL_ASK_PRICE//?>
										...
										<?} else {?>
											...
											if($inBtnBoc && !$arResult["COLLECTION"]["THIS"]) {?>
												<button id="<?=$arItemIDs['POPUP_BTN']?>" class="btn_buy boc_anch" data-action="boc"><span>Купить</span></button>
											<?}
											//DETAIL_CHEAPER
//                                            if($inBtnCheaper) {?>
...
<!--			</li>-->
		...
		<div class="tabs__box" <?=(!$arResult["COLLECTION"]["THIS"]) ? " style='display:block;'" : ""?>>
			<script>
                ...
                function setDetail() {
                    $('div.d_text').empty().html(<?=CUtil::PhpToJSObject($arResult["DETAIL_TEXT"], false, true);?>);
                }

                // setTimeout(setDetail, 50000);
                setDetail();
            </script>
            <div class="tabs__box-content d_text">
