...
<!--	<div class="item-image-cont">
    ...
    //                        $arFile = $rsFile->Fetch();
                            $picture = CFile::GetPath($photo);
    //                        $file = CFile::ResizeImageGet($arFile, array('width'=>$arElement['PREVIEW_PICTURE']['WIDTH'], 'height'=>$arElement['PREVIEW_PICTURE']['HEIGHT']), BX_RESIZE_IMAGE_EXACT, true); ?>
                            <div class="product_slider__elem">
                ...
                <?}else{?>
                    <div class="product_slider__elem">
                        <a class="item-title" href="<?=$arElement['DETAIL_PAGE_URL']?>" title="<?=$arElement['NAME']?>" itemprop="url">
                            <img class="item_img"
                                 data-lazy="<?=SITE_TEMPLATE_PATH?>/images/no-photo.jpg"
                                 width="150"
                                 height="150"
                                 alt="<?=$strAlt?>"
                                 title="<?=$strTitle?>" />
		...
		if($haveOffers) {
			...
			<?} else {?>
				<div class="item-price">
					...
					<?}?>
					<span class="catalog-item-price <?if($arElement['MIN_PRICE']['BASE_PRICE']!=$arElement['MIN_PRICE']['PRICE']) {echo 'skidka';}?>">
...
<!--				</div>-->
			<?} else {?>
				<div class="item-price item-price-discount-wrap">
...
//                pr($arElement);
					if($arElement["MIN_PRICE"]["RATIO_PRICE"] <= 0) {
					    if($arElement['PROPERTIES']['NOT_QUANTITY']['VALUE']){?>
					        <p  class="not-available"><?=Loc::getMessage("CT_BCS_ELEMENT_NOT_AVAILABLE")?></p>
                        <?}else{
                            //ITEM_ASK_PRICE//?>
        ...
        <?//BOC
        if($arElement["CAN_BUY"] || $arElement["TOTAL_OFFERS"]["QUANTITY"]>0){?>
<!--       <div class="boc-new">-->
<!--            <button type="button" id="--><?//=$itemIds["BTN_BOC"];?><!--" class="btn-boc"  data-action="boc" name="btn-boc">-->
<!--                <span>--><?//=Loc::getMessage('CT_BCS_ELEMENT_BOC')?><!--</span>-->
<!--            </button>-->
<!---->
<!--        </div>-->
<?}?>
