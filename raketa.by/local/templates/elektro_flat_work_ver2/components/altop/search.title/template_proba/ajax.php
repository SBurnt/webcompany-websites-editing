							...
							<?//ITEM_PRICE//
								...
								if($arElement["MIN_PRICE"]["CAN_ACCESS"]) {?>
									<div class="search_price">
                                        <?
                                        if($arElement["MIN_PRICE"]["CAN_ACCESS"]) {?>
                                            <div class="price_procent">
                                                <?
                                                if($arElement["MIN_PRICE"]["DISCOUNT_VALUE"] != $arElement["MIN_PRICE"]["PRINT_VALUE"]){?>
                                                ...
                                                <?
                                                if($inPercentPrice) {?>
                                                    <?if (!empty($arElement["MIN_PRICE"]["PRINT_DISCOUNT_DIFF"]!=0)):?>
                                                        <s>
                                                            <?=$arElement["MIN_PRICE"]["PRINT_VALUE"]?>
                                                        </s>
                                                    <?endif;?>
...
<!--													<span>--><?//=(!empty($arElement["CATALOG_MEASURE_NAME"])) ? GetMessage("CATALOG_ELEMENT_UNIT")." ".$arElement["CATALOG_MEASURE_NAME"] : "";?><!--</span>-->
										...
										<div class="add2basket_block">
											<?if($arElement["MIN_PRICE"]["DISCOUNT_VALUE"] <= 0) {
												...
												<form action="javascript:void(0)" class="apuo_form">
                                                    ...
                                                    <?if($arElement['PROPERTIES']['NOT_QUANTITY']['VALUE'] =="Нет в наличии"){?>
                                                        <p class="not-available"><?=GetMessage("CATALOG_ELEMENT_NOT_AVAILABLE")?></p>
                                                    <?}else{?>
    ...
    <div>
        <?
        $count_cat =0;
        foreach($arResult["CAT"] as $key_cat=>$value_cat){
            ?>
            <div class="tvr_search tvr_search_cat" style="<?if($count_cat>3) echo "display:none"?>">
                <div class="item_title_cat">
                    <a href="<?=$value_cat["SECTION_PAGE_URL"]?>"><?=$value_cat["NAME"]?></a>
                </div>
            </div>
            <?
            $count_cat++;
        }
        $count_cat_new = count($arResult["CAT"])-4;
        if($count_cat_new> 0) echo"<p class=\"search_all\" href=\"\">И ещё ".$count_cat_new."</p>";
        ?>
    </div>

	<?//JS//?>
	<script type="text/javascript">
        $("p.search_all").on("click",function () {
            $("div .tvr_search_cat").css("display","block");
            $("p.search_all").css("display","none");
        });
		BX.ready(function() {
			BX.message({
