...
$(function() {
	...
	$('.thumbs img').click(function(e){
		...
		$( '.thumbs a.fancybox' ).attr('data-fancybox', 'images');
		$( '#current' ).attr('src', $(e.target).attr('src'));
		$( '#current' ).closest('.fancybox').attr('href', $(e.target).data('full'));
		$(e.target).css('opacity', 1);
		$(e.target).closest('div').find('.fancybox').attr('data-fancybox', 'none');
	});
	
	$('.otm-btn').click(function() {
	setTimeout(function() {
		location.reload();
	}, 500);
	});
    ...
    $('.modal-window .avalaible.hidden').click(function(e){
        e.preventDefault();
        $('.modal-window').hide();
    });
