...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		getPersonTypeControl: function(node)
		{
			...
			{
				input = BX.create('DIV', {
					props: {className: 'checkout-buttons'},
					// children: [
			...
			else if (personTypesCount == 2)
			{
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						currentType = this.result.PERSON_TYPE[i];
						label = BX.create('LABEL', {
							props: {className: 'checkcontainer ways-nav physical-nav'},
							children: [
								...
								BX.create('SPAN', {
									props: {className: 'radiobtn', type: 'radio', name: 'PERSON_TYPE', value: currentType.ID}
								})
							],
							events: {change: BX.proxy(this.sendRequest, this)}
						});
						...
						node.appendChild(BX.create('A', {props: {href:'#', className: 'item__list--link'}, dataset: {target:"#ways-block"}, children: [label]}));
						delimiter = true;
