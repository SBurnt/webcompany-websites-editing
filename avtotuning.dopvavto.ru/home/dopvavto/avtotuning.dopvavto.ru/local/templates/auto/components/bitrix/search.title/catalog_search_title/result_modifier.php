...
if (!$arResult["SEARCH"] && !empty($arResult['query'])) {
    $arSelect = array(
        "ID",
        'NAME',
        "IBLOCK_ID",
    ...
    $poisk = '%'.$arResult['query'].'%';

//    $rsElements = CIBlockElement::GetList(array(), array('IBLOCK_ID' => 2, 'ACTIVE' => 'Y', 'PROPERTY_ARTICUL' => $arResult['query']), false, false, $arSelect);
    $rsElements = CIBlockElement::GetList(array(), array('IBLOCK_ID' => 2, 'ACTIVE' => 'Y', 'PROPERTY_ARTICUL' => $poisk), false, false, $arSelect);
    while($arElement = $rsElements->GetNext())
    {
        ...
        $arResult["FIND_ARTNUMBER"]['URL'] = '/search/index.php?q='.$arResult['query'];
