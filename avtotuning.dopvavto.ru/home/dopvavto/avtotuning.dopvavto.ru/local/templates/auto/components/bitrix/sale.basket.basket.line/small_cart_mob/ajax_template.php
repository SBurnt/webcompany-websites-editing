    ...
    <?foreach ($arResult["CATEGORIES"] as $category => $items):
        ...
        <?foreach ($items as $v):?>
            <li class="main_flex flex__align-items_center flex__jcontent_between">
                ...
                <?}?>
                <a href="<?=$v["DETAIL_PAGE_URL"]?>" class="web-basket-link">
                ...
                <?if($v['KOM']){?>
                    ...
                    if($v['PROPS_CART']['DISCOUNT_KIT_ACTIV']['VALUE'] == 'Y'){
                        foreach ($arFields3 as $KITS){?>
                            <a href="<?=((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] .'/catalog/'. $KITS["DETAIL_PAGE_URL"]?>" class="web-basket-link">
                            <span class="basket__open--img">
                                ...
                                <div class="basket__open--price"><?=$v["PRICE_FORMATED"]?></div>
                            </a>
                        <?}
                    }
                    if($v['PROPS_CART']['DISCOUNT_KIT_ACTIV']['VALUE'] != 'Y' && $v['PROPS_CART']['GIFT_SET_ACTIV']['VALUE'] == 'Y'){
                        foreach ($arFields2 as $GIFT){?>
                            <a href="<?=((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] .'/catalog/'. $GIFT["DETAIL_PAGE_URL"]?>" class="web-basket-link">
                            <span class="basket__open--img">
                        ...
                        <?}?>
                        <a href="<?=((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] .'/catalog/'. $arFields1["DETAIL_PAGE_URL"]?>" class="web-basket-link">
                            <span class="basket__open--img">
