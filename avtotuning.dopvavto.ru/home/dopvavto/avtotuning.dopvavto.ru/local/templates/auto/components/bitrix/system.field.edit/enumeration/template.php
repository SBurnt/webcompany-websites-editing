...
{
	?><select class="bx-user-field-enum ur" name="<?=$arParams["arUserField"]["FIELD_NAME"]?>" <?if($arParams["arUserField"]["SETTINGS"]["LIST_HEIGHT"] > 1)?>size="<?=$arParams["arUserField"]["SETTINGS"]["LIST_HEIGHT"]?>">
	<?
	...
	foreach ($arParams["arUserField"]["USER_TYPE"]["FIELDS"] as $key => $val)
	{
		...
		?><option value="<?echo $key?>"<?echo ($key==2? " selected" : "")?>><?echo $val?></option><?
	}
	?></select><?
}
