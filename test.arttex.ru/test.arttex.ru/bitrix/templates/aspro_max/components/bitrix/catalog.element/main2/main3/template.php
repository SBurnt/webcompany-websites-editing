...
<!--															--><?//echo '<pre>'.print_r($arResult['PROPERTIES']['EXTENDED_REVIEWS_RAITING']).'</pre>';?>

                                                            <meta itemprop="ratingValue" content="<?=!empty($arResult['PROPERTIES']['EXTENDED_REVIEWS_RAITING']['VALUE']) ? $arResult['PROPERTIES']['EXTENDED_REVIEWS_RAITING']['VALUE'] : 1?>" />
															<meta itemprop="reviewCount" content="<?=!empty($arResult['PROPERTIES']['EXTENDED_REVIEWS_COUNT']['VALUE']) ? intval($arResult['PROPERTIES']['EXTENDED_REVIEWS_COUNT']['VALUE']) : 5?>" />
							...
							<?if($arResult["OFFERS"] && $showCustomOffer):?>
								<?$frame = $this->createFrame()->begin('');?>
									<div class="buy_block offer-props-wrapper">
										<div class="sku_props">
<!--                                            --><?//echo '<pre>',print_r($arSkuTemplate,1),'</pre>';?>
<!--                                            --><?//echo '<pre>',print_r($arResult['OFFERS_PROP'],1),'</pre>';?>
...
//                            print_r($arResult['HELP_TEXT']);?>
		...
		<?
        if($arResult['OFFERS']):
//            echo '<pre>',print_r($arResult['OFFERS'][0]['BUY_URL']), '</pre>';
//            echo strstr($arResult['OFFERS'][0]['BUY_URL'], '?', true);
            ?>
...
<!--						<a href="--><?//=strstr($arOffer['BUY_URL'], '?', true)?><!--" itemprop="url"></a>-->
