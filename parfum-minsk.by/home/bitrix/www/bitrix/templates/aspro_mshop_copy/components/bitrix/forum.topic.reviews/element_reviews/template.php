...
foreach ($arResult["MESSAGES"] as $res):
	$iCount++;
?>
<?if ($iCount>0):?><script>$("#product_reviews_tab .count").removeClass("empty").html("&nbsp;(<?=$iCount?>)")</script><?endif;?>
...
<div class="reviews-collapse reviews-minimized" style='position:relative; float:none;'>
	<a class="button30 add_review reviews-collapse-link" onclick="fToggleCommentsForm(this)" href="javascript:void(0);">
		<span><?=$arParams['MINIMIZED_EXPAND_TEXT']?></span>
	</a>
</div>
<? } ?>
...
?>
		<div class="reviews-reply-buttons">
			<button name="send_button" type="submit" class="button30 send_review" value="<?=GetMessage("OPINIONS_SEND")?>" tabindex="<?=$tabIndex++;?>" <?
				?>onclick="this.form.preview_comment.value = 'N';"><span><?=GetMessage("OPINIONS_SEND")?></span></button>
			<button name="view_button" type="submit" class="button30 review_preview" value="<?=GetMessage("OPINIONS_PREVIEW")?>" tabindex="<?=$tabIndex++;?>" <?
				?>onclick="this.form.preview_comment.value = 'VIEW';"><span><?=GetMessage("OPINIONS_PREVIEW")?></span></button>
		</div>
