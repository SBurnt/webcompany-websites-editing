				...
				<a class="target" href="<?= $arItem["PROPERTIES"]["URL_STRING"]["VALUE"] ?>" <?= (strlen($target) ? 'target="' . $target . '"' : '') ?>>
					...
					<div class="wrapper_inner">
						<? 
								...
								?>
						<table class="slider_table" <?if($arItem["PREVIEW_PICTURE"]):?>style="background: url(<?= $arItem["PREVIEW_PICTURE"]["SRC"] ?>) <?= $position; ?> no-repeat"
							<?endif;?>>
							<tbody>
								<tr>
					...
					<?if ($arItem['PROPERTIES']["BTN_ON"]["VALUE_XML_ID"]=="Y"){?>
					<div class="wr_block">
						<span class="wrap_outer title">
							<a class="outer_text" href="<?= $arItem["PROPERTIES"]["BTN_LINK"]["VALUE"] ?>" <?= ($arItem["PROPERTIES"]["TARGETS"]["VALUE_XML_ID"] ? "target='" . $arItem["PROPERTIES"]["TARGETS"]["VALUE_XML_ID"] . "'" : ""); ?>>
								<span class="outer_text">
									<span class="inner_text">
										<img class="bg-img-red" src="bitrix/templates/aspro_mshop_copy/images/btn-red.png" alt="" />
										<img class="bg-img-red-hover" src="bitrix/templates/aspro_mshop_copy/images/btn-red-hover.png" alt="" />
										<?= strip_tags($arItem["PROPERTIES"]["BTN_TEXT"]["~VALUE"], "<br><br/>"); ?>
									</span>
									<?if($isUrl){?>
							</a>
							<?}else{?>
