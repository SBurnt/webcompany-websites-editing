...
if($section){
	...
	$posSectionDescr = COption::GetOptionString("aspro.mshop", "SHOW_SECTION_DESCRIPTION", "TOP", SITE_ID);
		...
		<div class="inner_wrapper">
			<?if($posSectionDescr=="TOP"){?>
				<?if ($arSection["SEO_DESCRIPTION"]):?>
					<div class="group_description_block  description_block_custom top">
				...
				<?endif;?>
                <span class="cat_desc_more">показать описание полностью</span>
			<?}?>
...
if($arTab){
    ...
    <?php
    if((isset($_REQUEST['PAGEN_1']) && $_REQUEST['PAGEN_1']>1) || ($_GET['sort'])){
