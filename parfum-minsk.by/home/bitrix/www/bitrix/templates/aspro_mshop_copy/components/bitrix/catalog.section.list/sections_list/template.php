...
<div class="catalog_section_list">
    <?foreach( $arResult["SECTIONS"] as $arItems ){
        ...
        $cur_letter = substr($arItems["NAME"], 0, 1);
        if($cur_letter != $prev_letter):
            $prev_letter = $cur_letter;?>
            <h2 class="catalog-letter"><?=$cur_letter?></h2>
            <hr class="long"/>
        <?endif;
        ?>
                    ...
                    <?if ($arParams["SHOW_SECTION_LIST_PICTURES"]=="Y"):?>
                        ...
                        <td class="image">
                            <?if($arItems["PICTURE"]["SRC"]):?>
                                <?$img = CFile::ResizeImageGet($arItems["PICTURE"]["ID"], array( "width" => 120, "height" => 120 ), BX_RESIZE_IMAGE_PROPORTIONAL, true );?>
                            ...
                            <?elseif($arItems["~PICTURE"]):?>
                                <?$img = CFile::ResizeImageGet($arItems["~PICTURE"], array( "width" => 120, "height" => 120 ), BX_RESIZE_IMAGE_PROPORTIONAL, true );?>
                                <a href="<?=$arItems["SECTION_PAGE_URL"]?>" class="thumb"><img src="<?=$img["src"]?>" alt="<?=($arItems["PICTURE"]["ALT"] ? $arItems["PICTURE"]["ALT"] : $arItems["NAME"])?>" title="<?=($arItems["PICTURE"]["TITLE"] ? $arItems["PICTURE"]["TITLE"] : $arItems["NAME"])?>" /></a>
                            <?else:?>
