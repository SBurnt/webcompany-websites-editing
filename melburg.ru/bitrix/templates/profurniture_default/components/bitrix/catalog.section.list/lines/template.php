...
foreach($arResult['SECTIONS'] as $key => $arSection){
	...
	if ($CURRENT_DEPTH < $arSection['DEPTH_LEVEL'] || !$CURRENT_DEPTH){
		?><ul class="<?if($CURRENT_DEPTH == $TOP_DEPTH){?>menu-sidebar clearfix one-line<?}?>"><?
...
while($CURRENT_DEPTH > $TOP_DEPTH){
	?></li><button class="menu-sidebar-more js-show-more-menu-sidebar js-invisible">Показать все</button></ul><?
	$CURRENT_DEPTH--;
}

$this->SetViewTarget('catalog_section_list_descr');
	if($arResult['SECTION']['DESCRIPTION']!='' && strpos($APPLICATION->GetCurPage(), '/filter/') ===false) {
		?><div class="sectinfo"><?
